...
if(isset($argv[1]))
{
    ...
    if(!file_exists($asset_dir)) {
        $logger->log(EventType::RECORDER_UPLOAD_WRONG_METADATA, LogLevel::CRITICAL, "Asset dir not found", array(__FILE__));
        exit(1);
    }
    
    $metadata_filepath = $asset_dir . '/metadata.xml';
    if(!file_exists($metadata_filepath)) {
        $logger->log(EventType::RECORDER_UPLOAD_WRONG_METADATA, LogLevel::CRITICAL, "Metadata file not found: $metadata_filepath", array(__FILE__));
        exit(1);
    }
    
    $meta_assoc = xml_file2assoc_array($metadata_filepath);
    if($meta_assoc == false) {
        $logger->log(EventType::RECORDER_UPLOAD_WRONG_METADATA, LogLevel::CRITICAL, "Could not get session metadata file, cannot continue (1)", array(__FILE__));
        exit(1);
    }
    ...
    $meta_assoc = $fct();
    if($meta_assoc == false) {
        $logger->log(EventType::RECORDER_UPLOAD_WRONG_METADATA, LogLevel::CRITICAL, "Could not get session metadata file, cannot continue (2)", array(__FILE__));
        exit(1);
    }
}
...
if (!file_exists($metadata_file)) {
    $logger->log(EventType::RECORDER_UPLOAD_WRONG_METADATA, LogLevel::CRITICAL, "Could not get asset metadata file from dir: $asset_dir, cannot continue", array(__FILE__), $asset);
...
if ($cam_enabled) {
    ...
    if($cam_download_info == false) {
        $logger->log(EventType::RECORDER_UPLOAD_TO_EZCAST, LogLevel::ERROR, "Couldn't get info from cam module. Camera will be ignored", array(__FILE__), $asset);
    }
}
...
if ($slide_enabled) {
    ...
    if($slide_download_info == false) {
        $logger->log(EventType::RECORDER_UPLOAD_TO_EZCAST, LogLevel::ERROR, "Couldn't get info from slide module. Slides will be ignored", array(__FILE__), $asset);
    }
}
...
if($new_record_type == false) { //we may have errors on both, stop in this case
    ...
    if(!$cam_info_ok && !$slide_info_ok) {
        $logger->log(EventType::RECORDER_UPLOAD_TO_EZCAST, LogLevel::CRITICAL, "All video streams modules are disabled or have errors, nothing to upload.", array(__FILE__), $asset);
        exit(3);
    }
...
$logger->log(EventType::RECORDER_UPLOAD_TO_EZCAST, LogLevel::INFO, "Starting upload to ezcast for asset $asset", array(__FILE__), $asset);
...
for($retry = 0; $retry < $retry_count; $retry++) {
    ...
    case 0: // no error, exit
        $logger->log(EventType::RECORDER_UPLOAD_TO_EZCAST, LogLevel::INFO, "Successfully sent upload request to ezcast.", array(__FILE__), $asset);
        //normal exit path
        
        exit(0);
    case 1: // error, retry
        $logger->log(EventType::RECORDER_UPLOAD_TO_EZCAST, LogLevel::ERROR, "Upload (curl) failed, will retry later.", array(__FILE__), $asset);
    ...
    case 2: // critical error, exit
        $logger->log(EventType::RECORDER_UPLOAD_TO_EZCAST, LogLevel::CRITICAL, "Upload failed after a critical error was encountered, giving up", array(__FILE__), $asset);
        exit(4);
    }
}

//if we get here, all retries have failed:
$logger->log(EventType::RECORDER_UPLOAD_TO_EZCAST, LogLevel::CRITICAL, "Upload failed after $retry_count tries, giving up", array(__FILE__), $asset);
exit(5);
...
function is_valid_cam_download_info($download_info, &$err_info = null) {
    ...
    if(!file_exists($download_info["filename"]))
    {
        ...
        $logger->log(EventType::RECORDER_UPLOAD_TO_EZCAST, LogLevel::DEBUG, __FUNCTION__.": $err_info", array(__FILE__));
        return false;
    }
