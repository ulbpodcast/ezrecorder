...
while (true) {
    ...
    //*
    if ($status == 'recording') {
        $startrec_time = private_capture_localqtb_starttime_get();
        $lastmod_time = private_capture_localqtb_lastmodtime_get();
        $now = time();
...
//sends a request to the 'main core' to let it know that a recording has timed out
    global $localqtb_force_quit_url;

    $ch = curl_init($localqtb_force_quit_url);
    $res = curl_exec($ch);
