...
$config = file_get_contents("$ffmpeg_basedir/config_sample.inc");

$config = preg_replace('/\$ffmpegpath = (.+);/', '\$ffmpegpath = "' . $ffmpegpath . '";', $config);
file_put_contents("$ffmpeg_basedir/config.inc", $config);
...
$bash_file = file_get_contents("$ffmpeg_basedir/bash/localdefs_sample");
$bash_file = str_replace("!PATH", $ffmpeg_basedir, $bash_file);
$bash_file = str_replace("!RECORD_PATH", $ffmpeg_recorddir, $bash_file);
$bash_file = str_replace("!MOVIES_PATH", $ffmpeg_moviesdir, $bash_file);
...
$bash_file = str_replace("!FFMPEG_MONO", ($ffmpeg_mono ? 1 : 0), $bash_file);
file_put_contents("$ffmpeg_basedir/bash/localdefs", $bash_file);
