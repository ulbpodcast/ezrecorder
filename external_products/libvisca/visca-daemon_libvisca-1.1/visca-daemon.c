...
void sig_abort(int signal_type) {
  ...
  /*Remove lock file*/
  unlink("/var/tmp/visca-daemon.lock");
	unlink("/var/tmp/visca-daemonchild.lock");
  if (background == TRUE) {
...
int main(int argc, char **argv) {
  ...
  if (stat("/var/tmp/visca-daemon.lock", &tempstat) == 0) {
    fprintf(stderr, "Lockfile /var/tmp/visca-daemon.lock exists\nMaybe another visca-daemo is running already?\n\n");
    exit(1);      
  }
  ...
  mknod("/var/tmp/visca-daemon.lock", S_IFREG, 0);
