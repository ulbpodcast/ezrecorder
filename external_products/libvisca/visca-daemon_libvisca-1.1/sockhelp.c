...
int *listener;
{
  ...
  /* 15feb2010MJ limit reach to localhost
      ...
      if (new_process < 0) {
        ...
        if (new_process == 0) {
          /* This is the new process. */
			struct stat tmpstat;
			if(stat("/var/tmp/visca-daemonchild.lock", &tmpstat) == 0) {
				// Lock exists: there is already another process running
				sock_puts(connected_socket, "Ressource already in use");
				return -1;
			}
			else {
				// Lock does not exist: we run the new process
				open("/var/tmp/visca-daemonchild.lock", O_CREAT | O_TRUNC | O_EXCL | O_RDONLY, S_IRWXU | S_IRGRP | S_IROTH);
