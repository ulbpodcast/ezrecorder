...
{
int i;
uint32_t nbunreadbytes;
unsigned char buffer[2048];
  //removes unread bytes first
   VISCA_unread_bytes(iface, buffer, &nbunreadbytes);

#ifdef DEBUG
   fprintf(stderr,"sending:");
   for(i=0;i<packet->length;i++) fprintf(stderr,"%02x  ",packet->bytes[i]);
   fprintf(stderr,"\n");
   fprintf(stderr,"reply:");
   for(i=0;i<10;i++) fprintf(stderr,"%02x  ",iface->ibuf[i]);
        fprintf(stderr,"\n");
#endif
...
VISCA_set_pantilt_absolute_position(VISCAInterface_t *iface, VISCACamera_t *camera, uint32_t pan_speed, uint32_t tilt_speed, int pan_position, int tilt_position)
{
  ...
  _VISCA_append_byte(&packet, pan_speed);
  _VISCA_append_byte(&packet, 0);/*BRC300 specific */

  _VISCA_append_byte(&packet, (pan_pos & 0xf0000) >> 16);/*BRC300 specific */
...
VISCA_get_pantilt_position(VISCAInterface_t *iface, VISCACamera_t *camera, int *pan_position, int *tilt_position)
{
  ...
  if (err!=VISCA_SUCCESS)
    ...
    {
      pan_pos  = ((iface->ibuf[2] & 0xf) << 16) + ((iface->ibuf[3] & 0xf) << 12) + ((iface->ibuf[4] & 0xf) << 8) + ((iface->ibuf[5] & 0xf) << 4) + (iface->ibuf[6] & 0xf); 
